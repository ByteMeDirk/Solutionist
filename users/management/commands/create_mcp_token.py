...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--name", type=str, default="API Access", help="Name for the token"
        )
        parser.add_argument(
            "--days", type=int, default=365, help="Days until token expires"
        )
    ...
    def handle(self, *args, **options):
        ...
        # Create token
        token = MCPToken(
            user=user, name=token_name, expires_at=timezone.now() + timedelta(days=days)
        )
        token.save()
