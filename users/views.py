...
def user_profile_view(request, username):
    ...
    solutions = (
        Solution.objects.filter(author=viewed_user, is_published=True)
    ...
    # Get most used tags
    top_tags = (
        Tag.objects.filter(solutions__author=viewed_user, solutions__is_published=True)
        .annotate(usage_count=Count("solutions"))
    ...
    top_rated_solutions = (
        solutions.annotate(avg_rating=Avg("ratings__value"))
...
def account_delete_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # First handle related models to avoid integrity errors
            ...
            # Try to handle MCPToken deletion safely
            try:
                from .mcp import MCPToken

                MCPToken.objects.filter(user=user).delete()
            except Exception:
                # Silently ignore if MCPToken model doesn't exist or table is missing
                pass
            ...
            # Delete the user using the stored ID
            User.objects.filter(id=user_id).delete()

            messages.success(
                request, f"Account for {username} has been deleted successfully."
            )
...
def create_mcp_token(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            "MCP token created successfully. Please copy your token now, you won't be able to see it again.",
        )
        return redirect("users:mcp_tokens")
...
def settings_view(request):
    ...
    return render(
        request, "users/settings.html", {"form": form, "active_tab": "settings"}
    )
