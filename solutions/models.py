...
from .ratings import Rating  # Import Rating from ratings.py instead of defining it here
...
class Solution(models.Model):
    ...
    summary = models.TextField(
        editable=False, blank=True, help_text="AI-generated summary of the solution"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            not self.pk
            or "content" in kwargs.get("update_fields", [])
            or kwargs.get("force_summary", False)
        ):
...
class SolutionVersion(models.Model):
    ...
    solution = models.ForeignKey(
        Solution, on_delete=models.CASCADE, related_name="versions"
    )
    content = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)
