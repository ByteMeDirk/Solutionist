...
class CommentViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.add_comment_url = reverse('comments:add_comment', args=[self.solution.slug])
        self.add_reply_url = reverse('comments:add_reply', args=[self.solution.slug, self.comment.id])
        self.delete_comment_url = reverse('comments:delete_comment', args=[self.comment.id])
    ...
    def test_add_reply_view(self):
        ...
        self.client.login(username='testuser', password='testpass123')
        response = self.client.post(self.add_reply_url, {
            'content': 'New reply'
        })
        self.assertEqual(response.status_code, 302)  # Redirect after success
        self.assertEqual(Comment.objects.count(), 2)
    ...
    def test_comment_view_authentication(self):
        ...
        response = self.client.post(self.delete_comment_url)
        self.assertEqual(response.status_code, 404)  # Not found, as the comment doesn't belong to this user
        self.assertEqual(Comment.objects.count(), 1)  # Comment count unchanged
